//
// ASCOM.OC61domeServer2 Local COM Server
//
...
namespace ASCOM.OC61domeServer2
{
    public static class Server
    {
        ...
        private static string s_appId = "{e2d85660-7790-466d-9820-d21c7887b008}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "OC61domeServer2", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The OC61domeServer2 was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "OC61domeServer2", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "OC61domeServer2", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "OC61domeServer2", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
